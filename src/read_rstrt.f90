!234567
  ...
  open (18,file=trim(cfn)//'/u_base.'//cidx,form="unformatted", &
           access="direct",action='read',status='old',recl=length)
        read (18,rec=1) u
  write(*,*) '   * Reading u-component... done'
  close(18)

  open (18,file=trim(cfn)//'/v_base.'//cidx,form="unformatted", &
           access="direct",action='read',status='old',recl=length)
        read (18,rec=1) v
  write(*,*) '   * Reading v-component... done'
  close(18)

  open (18,file=trim(cfn)//'/w_base.'//cidx,form="unformatted", &
           access="direct",action='read',status='old',recl=length)
        read (18,rec=1) w
